...
def _upsert_column(
    ...
    *,
    ...
    """Append a new column or replace an existing one in-place."""
    matching_indices = [
        idx for idx, column_name in enumerate(table.column_names) if column_name == name
    ]

    if len(matching_indices) > 1:
        raise ValueError(
            f"Parquet file already contains duplicate '{name}' columns: {source_path}. "
            "Please restore a clean dataset copy before re-running augment."
        )

    if matching_indices:
        if not overwrite:
            raise RuntimeError(
                f"Column '{name}' already exists in {source_path}; use --overwrite to replace it."
            )
        return table.set_column(matching_indices[0], pa.field(name, values.type), values)

    return table.append_column(pa.field(name, values.type), values)
...
def merge_datasets(
    ...
    Args:
        ...
        merge_custom_meta: Whether to merge custom meta files (episode_metadata.json)
    """
    ...
    if merge_custom_meta:
        logger.info("Merging episode metadata files...")
        merge_episode_metadata(source_roots, output_root)
        logger.info("Episode metadata files merged successfully")

    logger.info("Dataset merge completed!")


def merge_episode_metadata(source_roots: List[Path], output_root: Path) -> int:
    """Merge episode_metadata.json files from multiple datasets.
    ...
    """
    output_path = output_root / "meta" / "episode_metadata.json"
    output_path.parent.mkdir(parents=True, exist_ok=True)
    ...
    for source_root in source_roots:
        source_path = source_root / "meta" / "episode_metadata.json"

        if not source_path.exists():
            logger.warning(f"Metadata file not found in {source_root}, skipping...")
        ...
        logger.info(f"Merging metadata from {source_root} ({source_path.name})")
        count = 0
        ...
        try:
            ...
            for variant_name, episodes in source_data.items():
                if variant_name not in merged_data:
                    merged_data[variant_name] = {}

                for episode_key, episode_data in episodes.items():
                    try:
                        old_idx = int(episode_key)
                        new_key = str(old_idx + episode_offset)
                        merged_data[variant_name][new_key] = episode_data.copy()
                        count += 1
    ...
    # Sort by variant_name and episode indices
    sorted_data = {}
    for variant_name in sorted(merged_data.keys()):
        episodes = merged_data[variant_name]
        sorted_episodes = {
            str(k): episodes[str(k)]
            for k in sorted(int(key) for key in episodes.keys())
        }
        sorted_data[variant_name] = sorted_episodes

    with open(output_path, "w", encoding="utf-8") as f:
        json.dump(sorted_data, f, indent=2, ensure_ascii=False)

    logger.info(
        f"Total merged {total_merged} episodes from {len(sorted_data)} variants to {output_path}"
    )
    return total_merged
