...
def print_meta_info(dataset_root: Path) -> Optional[Dict[str, Any]]:
    """Display dataset metadata information including episode_metadata.json."""
    print_separator()
    ...
    # Support variant metadata
    metadata_path = meta_dir / "episode_metadata.json"
    ...
    if metadata_path.exists():
        try:
            ...
            print_separator("-")
            print(f"📋 Task Variant Information (from {metadata_path.name})")
            print_separator("-")

            total_episodes_in_meta = 0
            for variant_name, episodes in metadata.items():
                episode_count = len(episodes)
                total_episodes_in_meta += episode_count
                print(f"  • {variant_name}: {episode_count} episode(s)")
            ...
            print(f"\n  Total episodes in variant metadata: {total_episodes_in_meta}")

            total_episodes_info = info.get("total_episodes", 0)
            if total_episodes_in_meta != total_episodes_info:
                print(f"  ⚠️  Warning: Episode count mismatch!")
                print(
                    f"     info.json: {total_episodes_info}, {metadata_path.name}: {total_episodes_in_meta}"
                )
            else:
                print(f"  ✅ Episode count matches info.json")

        except json.JSONDecodeError as e:
            print(f"  ❌ Failed to parse {metadata_path.name}: {e}")
        except Exception as e:
            print(f"  ❌ Error reading {metadata_path.name}: {e}")
    else:
        print_separator("-")
        print("⚠️  No variant metadata found (episode_metadata.json missing)")
