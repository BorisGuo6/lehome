...
class RobotKinematics:
    ...
    def _init_pinocchio(self):
        ...
        missing_attrs = [name for name in required_attrs if not hasattr(pin, name)]
        if missing_attrs:
            module_path = getattr(pin, "__file__", "<unknown>")
            module_version = getattr(pin, "__version__", "<unknown>")
            raise ImportError(
                "Imported a 'pinocchio' module, but it is not the robotics "
                f"Pinocchio library. Missing attributes: {missing_attrs}. "
                f"Loaded module: {module_path} (version: {module_version}). "
                "This usually means the wrong PyPI package 'pinocchio' was "
                "installed. Please uninstall 'pinocchio' and install the "
                "robotics package with: pip uninstall pinocchio && pip install pin"
            )
