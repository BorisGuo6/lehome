...
def cut_prim(stage, prim, plane_origin, plane_normal):
    dependencies = ["trimesh", "shapely", "rtree", "triangle"]
    missing_deps = []
    ...
    if missing_deps:
        print(f"Missing dependencies detected: {missing_deps}. Attempting auto-installation...")
        for package_name in dependencies: # Install all to be safe if any are missing
            install_package(package_name)
        # Verify again or advise restart
        raise RuntimeError(f"Dependencies {missing_deps} were missing and attempted to install. Please restart the script.")
...
def install_package(package_name, install_name=None):
    """
    Installs a package using uv (preferred) or pip.
    """
        ...
        if os.path.exists(whls_dir):
            for file in os.listdir(whls_dir):
                if file.startswith(install_name) and file.endswith(".whl"):
                    if current_os == "Windows" and ("win" in file.lower() or "py3-none-any" in file.lower()):
                        package_whl = os.path.join(whls_dir, file)
                        break
                    elif current_os == "Linux" and ("linux" in file.lower() or "manylinux" in file.lower() or "py3-none-any" in file.lower()):
                        package_whl = os.path.join(whls_dir, file)
                        break

        install_target = package_whl if (package_whl and os.path.exists(package_whl)) else install_name
        
        if uv_path:
            print(f"Using uv to install {install_name}...", flush=True)
            cmd = [uv_path, "pip", "install", install_target, "--python", python_exe]
            if not package_whl:
                cmd += ["-i", "https://mirrors.aliyun.com/pypi/simple/", "--trusted-host", "mirrors.aliyun.com"]
            subprocess.run(cmd, check=True)
        else:
            print(f"uv not found, falling back to pip for {install_name}...", flush=True)
            cmd = [python_exe, "-m", "pip", "install", install_target]
            if not package_whl:
                cmd += ["-i", "https://mirrors.aliyun.com/pypi/simple/", "--trusted-host", "mirrors.aliyun.com"]
            subprocess.run(cmd, check=True)
