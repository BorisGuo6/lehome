...
class Se3Keyboard(Device):
    ...
    def _on_keyboard_event(self, event, *args, **kwargs):
        ...
        if event.type == carb.input.KeyboardEventType.KEY_PRESS:
            # Canonicalize key name for comparison
            key_canonical = key_name.upper()
            if key_canonical.startswith("KEY_"):
                key_canonical = key_canonical[4:]

            if key_name in self._INPUT_KEY_MAPPING.keys():
                self._delta_pos += self._INPUT_KEY_MAPPING[key_name]

            # Functional keys - ensure they work inside IsaacSim window
            elif key_canonical == "B":
                self.started = True
                self._reset_state = False
                print(">> [INFO] CONTROL STARTED!")
            elif key_canonical == "S":
                if "S" in self._additional_callbacks:
                    self._additional_callbacks["S"]()
            elif key_canonical == "D":
                if "D" in self._additional_callbacks:
                    self._additional_callbacks["D"]()
            elif key_canonical == "N":
                if "N" in self._additional_callbacks:
                    self._additional_callbacks["N"]()
            elif key_canonical in ["ESCAPE", "ESC"]:
                if "ESCAPE" in self._additional_callbacks:
